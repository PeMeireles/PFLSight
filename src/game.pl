...
handle_move([Board, Next, P1, _], Moves, NewState) :-
  Next = a, 
  (P1 = 0 ->
      ...
      ;
      choose_move([Board, Next], P1, Move),
      move_pre_val([Board,Next], Move, NewState, Moves)
  ), !.

handle_move([Board, Next, _, P2], Moves, NewState) :-
  Next = b, 
  (P2 = 0 ->
      (check_no_stacks(Moves) ->
          handle_new_piece([Board, Next], Moves, NewState)
      ;
         handle_regular_move([Board, Next], Moves, NewState))
      ;
      choose_move([Board, Next], P2, Move),
      move_pre_val([Board,Next], Move, NewState, Moves)
  ), !.
